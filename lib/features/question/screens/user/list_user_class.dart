...
class ListUserClass extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: LAppBar(title: Text('List User In Class'), 
        showBackArrow: true,
        actions: [IconButton(onPressed: () => Get.to(() => ListUser(course: course,)), icon: Icon(Iconsax.element_plus))],
      ),
                ...
                itemBuilder: (_, index) {
                  return Row(
                    ...
                    children: [Expanded(child: LUserCard(user: users![index])),
                      IconButton(icon: const Icon(Iconsax.minus, color: LColors.primary1,),
                        onPressed: () => classController.removeUserFromClass(users[index], course.id!),)
                    ],
                  );
                },);
              // Data found
